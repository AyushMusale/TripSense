...
class TripcreationHandler extends Bloc<TripEvents, TripcreationState>{
  ...
  TripcreationHandler(this._tripcreationusecase) : super(TripCreationInitial()){
    on<TripcreationEvent>((event , emit)async{
        emit(TripCreationInproccess());
        try{
          final res =  await _tripcreationusecase.execute(event.date, event.destination);
          final tripID = res.id;
          final error = res.errormsg;
          if(tripID != null){
              emit(TripCreationSuccess());
          }
          else{
